...
from pixelcnnpp.discretized_mix_logistic import discretized_mix_logistic_loss, sample_image
from pixelcnnpp.model import PixelCNNpp


def train():
    config = OmegaConf.load('./pixelcnnpp/config.yaml')
    ...
    transforms = T.Compose(
        [
            T.ToImage(),
            T.ToDtype(dtype=torch.float32, scale=True),  # We use the RGB values as labels.
    ...
    # Create model.
    model = PixelCNNpp(**OmegaConf.to_object(config.model))
...
def train_loop(
    ...
    while batches_done < config.train.num_iterations:
        ...
        for batch in dataset:
            ...
            # Loss.
            recon_loss = discretized_mix_logistic_loss(output, image, n_bits=4)
            batch_loss = recon_loss.mean()
            ...
            if batches_done % config.train.save_every == 0:
                ...
                images = sample_image(model, image.size(), device)
