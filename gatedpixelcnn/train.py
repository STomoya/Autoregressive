...
from pixelcnn.model import sample_image


def train():
    config = OmegaConf.load('./gatedpixelcnn/config.yaml')
    ...
    logger = torchutils.get_logger(
        f'GatedPixelCNN [{torchutils.get_rank()}]',
    ...
    # Create model.
    model = GatedPixelCNN(**OmegaConf.to_object(config.model))
